...
class CustomTag:
    ...
    def delete(cls, tag_id: str, project_id: str):
        ...
        if response is not None:
            tag = CustomTagModel(id=response["id"], type=response["type"])
    ...
    def create(cls, project_id: str, tag_name: str):
        ...
        """
        url = "customdata/tags"
        payload = {"data": {"name": tag_name, "project_uid": project_id}}
        response = Client.post(url, payload)

        if response is not None:
            tag = CustomTagModel(id=response["id"], name=response["name"])
